...
BUSY1	LDR	R1, =SSI0_SR_R
		LDR	R2, [R1]
		AND	R2, #0x10
		CMP	R2, #0x10;is bit 4 of SSI0_SR_R high
		BEQ	BUSY1;continue if not BUSY
		LDR	R1, =DC
		LDR	R2, [R1]
		...
		LDR	R2, [R1]
		MOV	R2, R0
		STR	R2, [R1];move command into SSI0_DR_R
BUSY2	LDR	R1, =SSI0_SR_R
		LDR	R2, [R1]
		AND	R2, #0x10
		CMP	R2, #0x10;is bit 4 of SSI0_SR_R high
		BEQ	BUSY2;end if not BUSY
		BX  LR                          ;   return
...
BUSY3	LDR	R1, =SSI0_SR_R
		LDR	R2, [R1]
		AND	R2, #0x02
		CMP	R2, #0x02;is bit 1 of SSI0_SR_R high
		BNE	BUSY3;if TNF is high, continue
		LDR	R1, =DC
		LDR	R2, [R1]
		ORR	R2, #0x40
		STR	R2, [R1];set PA6 to 1
		LDR	R1, =SSI0_DR_R
		LDR	R2, [R1]
		MOV	R2, R0
		STR	R2, [R1];move data into SSI0_DR_R
		BX  LR                          ;   return
